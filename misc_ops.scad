...
// arrange child objects in an MxN grid
module grid(M=1, N=1, ax=U*[$x,$y],center=false) {
    let (
        ML = center? -(M-1)/2 : 0, 
        MR = center?  (M-1)/2 : M-1,
        NB = center? -(N-1)/2 : 0,
        ...
        for(i=[ML:MR], j=[NB:NT]) 
        move((i*axes[0] +j*axes[1])) children(); 
    }
}

// move to polar coordinate z = '[r,w]'  !>: need to add 2D | 3D tests
module rmove(z, rot=true) { move(P2R(z)) rotate(rot ? z[1]:0) children();}

// instantiate 'N' children along an arc, from R0 in increments of dR (all in polar form)
module arcn(R0,dR,N,rot=true){ for(i=[0:N-1]) rmove(R0+i*dR, rot)children();}
...
module arcl(R0,dR,RN,rot=true){ arcn(R0, dR, (RN[1]-R0[1])/dr[1],rot) children();}
 ...
 module rgrid(R0,dR,M,N,center=true) {
    let( LR = center? ctr(0,M-1) : [0,M], BT = center? ctr(0,N-1) : [0,N] ) {
        for (i=[LR[0]-LR[1]:(LR[0]+LR[1])], j=[BT[0]-BT[1]:BT[0]+BT[1]])
        let( r=[R0[0]+j*dr[0], R0[1]+i*dR[1]])  rmove(r) children();
    }
}
     
module fan(R0,dR,RN,center=true) 
    { let (M=(RN[1]-R0[1])/dR[1]+1, N=((RN[0]-R0[0])/dR[0]+1)) rgrid(R0,dR,M,N,center) children();}
    
rgridp([2,45], [.5,5 ], [4,90]) square([.15,.1],center=true);
...
//rgrid(r0,dr,5,4) square([.1,.1],center=true);
