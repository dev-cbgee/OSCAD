...
BPP = [zero, [BPW,0], [h3,h1], [h3,h1+h2], [0,BPH]];                // baseplate point list
BPL = concat([for (i=[0:1: len(BPP)-2]) BPP[i]], [[0,h1+h2+h4]]);   // ?!?!  stacking lip point list
...
//negative(BPP, [1,1],[.5,1.5]);  // displaced & scaled in 2 dimensions
//poly(5, [1,1],[.75,.5]);        // pentagon,  displaced & scaled in 2 dimensions
poly(BPp);


//snippet(BPP, llc=[0,4], sz=[1,1]);

//extrude() negative(BPP);
